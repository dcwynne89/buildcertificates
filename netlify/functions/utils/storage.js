/* ============================================================
   ...
   BuildCertificates — uses bcrt_ prefix + certificate tier quotas
   ============================================================ */
...
function generateApiKey() {
  const crypto = require("crypto");
  return "bcrt_" + crypto.randomBytes(24).toString("base64url");
}
...
async function validateKey(apiKey) {
  if (!apiKey || !apiKey.startsWith("bcrt_")) return null;
  const store = getConfiguredStore(KEYS_STORE);
  const keyHash = await hashKey(apiKey);
  try {
...
async function checkQuota(keyHash, tier) {
  ...
  const limit = tier.certsPerMonth;
